...
const MAXIMUM_PAGES = 128;   // 4MB maximum

// class MandelbrotCanvas {
//   constructor(liveViewHook) {
//     this.hook = liveViewHook;
//     this.canvas = liveViewHook.el;
...
//     const bytesNeeded = cols * rows * 4; // 4 bytes per pixel (RGBA)
...
//       console.error(`Canvas size requires ${initialPages} pages, but maximum is ${MAXIMUM_PAGES}`);
...
//       Initial memory: ${(initialPages * WASM_PAGE_SIZE / 1024 / 1024).toFixed(2)}MB
//       Maximum memory: ${(MAXIMUM_PAGES * WASM_PAGE_SIZE / 1024 / 1024).toFixed(2)}MB
//     `);
//   }
    ...
    async mounted() {
      try {
       ...
       imax = 100;  // max iterations
        ...
        // Set up event listeners for zooming, etc.
    ...
    setupEventListeners() {
        this.canvas.addEventListener("click", (e) => {
            ...
            this.wasm.exports.initialize(
                ...
                100,                 // imax (iteration max)
                newBounds.min_x,     // topleft_x
                newBounds.max_y,     // topleft_y
                newBounds.max_x,     // bottomright_x
                newBounds.min_y      // bottomright_y
            );
